...
int getV1APIinfo(const char * vidid)
{
	...
	char* apiKeyEnd;
	if (apiKeyStart) apiKeyStart = strchr(apiKeyStart, ':')+1;
	if (apiKeyStart) apiKeyEnd = strchr(++apiKeyStart, '\"');
	...
	fclose(f);
	if (ContinuationStart) ContinuationStart = strchr(ContinuationStart, ':') + 1;
	
	if (ContinuationStart) ContinuationEnd = strchr(++ContinuationStart, '\"');
